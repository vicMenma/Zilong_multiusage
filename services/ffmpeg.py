...
Key correctness fixes vs original:
  - trim_video: uses -ss AFTER -i for frame-accurate cut, -t for duration (not -to)
  - split_video: consistent with trim_video fix
  - merge_videos: correct concat-demuxer path escaping (backslash, not shell-quote)
  - ffprobe_streams: 3-pass with doubled params + mediainfo JSON fallback
  - ffprobe_duration: 4-strategy cascade
  - get_thumb: brightness measured via PIL if available, else file-size heuristic;
    never calls ffprobe signalstats on a JPEG
  - all subprocesses check returncode and capture stderr
"""
...
async def _run(cmd: list, label: str = "FFmpeg") -> None:
    """
    Run a command. Raises RuntimeError with last 1200 chars of stderr on failure.
    """
...
async def _probe_json(args: list) -> Optional[dict]:
    """Run ffprobe, return parsed JSON or None."""
...
async def probe_streams(path: str) -> dict:
    """
    Return {"video": [...], "audio": [...], "subtitle": [...]} ffprobe stream dicts.
    Pass 1: standard extended probe
    Pass 2: doubled analyzeduration / probesize
    Pass 3: mediainfo --Output=JSON fallback
    """
    ...
    # Pass 3 — mediainfo JSON fallback
...
async def probe_duration(path: str) -> int:
    """
    4-strategy cascade. Returns integer seconds, 0 if unknown.
    Strategy 1: format.duration
    Strategy 2: max stream.duration (numeric)
    Strategy 3: stream tags DURATION (HH:MM:SS.mmm)
    Strategy 4: nb_frames / avg_frame_rate
    """
    ...
    max_dur = 0.0
    for s in data.get("streams", []):
        # Strategy 2 — numeric stream.duration
        raw = s.get("duration")
        if raw:
            try:
                v = float(raw)
...
def _jpeg_brightness(path: str) -> float:
    """
    Estimate brightness from a JPEG using PIL (if available).
    Falls back to file-size heuristic.
    Does NOT call ffprobe — signalstats is a video filter, not for JPEGs.
    """
    try:
        ...
        pass
    # File-size heuristic: a very dark frame compresses to < 3 KB
    try:
...
async def get_thumb(path: str, out_path: str) -> Optional[str]:
    """
    Extract a non-dark thumbnail.
    Tries 20 / 30 / 40 / 50 / 10 / 5 / 1% of duration.
    Returns out_path on success, or the last extracted path even if dark,
...
async def video_meta(path: str) -> dict:
    """Return {duration, width, height, thumb} for a video file."""
    meta = {"duration": 0, "width": 0, "height": 0, "thumb": None}
    data = await _probe_json([
        "-select_streams", "v:0",
        "-show_entries", "stream=width,height,duration,r_frame_rate:stream_tags=rotate:format=duration",
        path,
    ])
    if data:
        for s in data.get("streams", []):
...
async def get_mediainfo(path: str) -> str:
    """Try mediainfo CLI, fall back to ffprobe-based text."""
    try:
        proc = await asyncio.create_subprocess_exec(
            "mediainfo", path,
            stdout=asyncio.subprocess.PIPE,
...
async def trim_video(inp: str, out: str, start: str, end: str) -> None:
    """
    Frame-accurate trim.
    -ss AFTER -i (slow but accurate), -to gives absolute end time.
    Use this when accuracy matters (trim tool).
    For a fast split with slight inaccuracy use trim_video_fast().
    """
    await _run([
        "ffmpeg","-y",
        "-i", inp,
...
async def trim_video_fast(inp: str, out: str, start_sec: float, duration_sec: float) -> None:
    """
    Fast keyframe-seek trim for split_video.
    -ss BEFORE -i (fast), -t is DURATION (not end time — avoids the off-by-N bug).
    """
    await _run([
        "ffmpeg","-y",
        "-ss", str(start_sec),
...
async def split_video(inp: str, out_dir: str, chunk_sec: int) -> list[str]:
    """
    Split into chunk_sec-second parts using fast keyframe seek.
    Uses trim_video_fast (duration-based) to avoid the -to offset bug.
    """
...
async def merge_videos(paths: list[str], out: str, tmp_dir: str) -> None:
    ...
    Correct path escaping for the concat list (not shell escaping).
    """
    list_file = os.path.join(tmp_dir, "concat_list.txt")
    with open(list_file, "w", encoding="utf-8") as f:
        for p in paths:
            # Concat demuxer escaping: backslash before single-quote and backslash
...
async def stream_op(inp: str, out: str, map_args: list) -> None:
    """Generic stream-copy operation (map / remove / extract)."""
