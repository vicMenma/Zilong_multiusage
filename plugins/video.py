...
Architecture:
  - Every callback acquires session.lock before starting work
    → no concurrent FFmpeg ops on the same file
  - _ensure() downloads the file once and caches local_path
  - All FFmpeg calls are in services/ffmpeg.py (tested, typed)
  - No NameError risks: user_id bound at top of every callback
"""
...
# Commands that must NOT trigger the text-reply handler
_IGNORED = {
    ...
    "show_thumb","del_thumb","json_formatter",
}
...
async def _tracked_ffmpeg(user_id: int, label: str, fname: str, coro) -> None:
    """Run an FFmpeg coroutine and register it in the global tracker."""
...
async def _ensure(client: Client, session: FileSession, st) -> str | None:
    """Download if not already cached. Thread-safe via session.lock."""
...
async def video_cb(client: Client, cb: CallbackQuery):
    ...
    # ── Actions that only prompt (no download) ────────────────
    prompts = {
    ...
    async with session.lock:
        ...
        try:
            ...
            log.error("video_cb action=%s: %s", action, exc)
...
async def _execute(client, cb, action, key, session, user_id,
    ...
    """All actions that require the downloaded file."""
    ...
    # ── Stream menus ──────────────────────────────────────────
    elif action in ("sext_menu","smap_menu","srem_menu"):
        label_map = {"sext_menu":"📤 Stream Extractor","smap_menu":"🗺️ Stream Mapper","srem_menu":"🗑️ Stream Remover"}
    ...
    # ── Screenshots ───────────────────────────────────────────
        ...
        await _tracked_ffmpeg(user_id, "Screenshots", session.fname,
                              FF.screenshots(path, tmp, count=5))
        shots = [os.path.join(tmp, f) for f in os.listdir(tmp)
    ...
    if action == "merge_av":
        ...
        prompts = {
            ...
            "split": (
                ...
                "Send chunk size in seconds: <code>600</code>"
            ),
            "sample": (
        ...
        await safe_edit(st, prompts[action], parse_mode=enums.ParseMode.HTML)
...
async def stream_cb(client: Client, cb: CallbackQuery):
    action_raw, idx_str, key = cb.data.split("|", 2)
    user_id = cb.from_user.id
    session = sessions.get(key)
    if not session:
        ...
        try:
            if action_raw == "smap":
                ...
                if idx_str == "all":
                    ...
                    await FF.stream_op(path, out, ["-map","0",f"-map",f"-0:{idx_str}","-c","copy"])
                await upload_file(client, st, out)
...
# ─────────────────────────────────────────────────────────────
# Audio format callback  vaud|<fmt>|<key>
# ─────────────────────────────────────────────────────────────
...
async def audio_fmt_cb(client: Client, cb: CallbackQuery):
    ...
    async with session.lock:
        ...
        if not path:
            return
        out = os.path.join(session.tmp_dir, os.path.splitext(os.path.basename(path))[0] + f".{fmt}")
        ...
        try:
            ...
            return await safe_edit(st, f"❌ Audio conversion failed: <code>{exc}</code>",
                                   parse_mode=enums.ParseMode.HTML,
                                   reply_markup=video_menu_kb(key))
        await upload_file(client, st, out)
...
# ─────────────────────────────────────────────────────────────
# Video convert callback  vconv|<fmt>|<key>
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# Optimize callback  vopt|<crf>|<key>
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# Secondary file handler (merge_av / merge_vs / merge_vids)
# ─────────────────────────────────────────────────────────────
...
async def handle_secondary_file(client: Client, msg: Message, session: FileSession):
    ...
    if action == "merge_av":
        ...
        out   = os.path.join(tmp, f"{base}_{'subbed' if action=='merge_vs' else 'burned'}{ext}")
