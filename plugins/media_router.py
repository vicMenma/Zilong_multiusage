...
No circular imports — plugins register their own handlers;
this router only creates the session and routes to the correct builder.
"""
import os
import logging
...
async def media_router(client: Client, msg: Message):
    ...
    if waiting_session and waiting_session.waiting in (
        "merge_av", "merge_vs", "merge_vids", "fwd_file", "archive_collect"
    ):
    ...
    await msg.reply(
        ...
        "<i>File type not supported for processing.</i>",
        parse_mode=enums.ParseMode.HTML,
    )
    msg.stop_propagation()
...
async def _open_audio_session(client, msg, media, fname, fsize, ext, uid):
    ...
    await msg.reply(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🎵 Convert",    callback_data=f"vaud|mp3|{key}"),
